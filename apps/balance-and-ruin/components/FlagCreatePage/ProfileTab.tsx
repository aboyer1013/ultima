...
export const ProfileTab = () => {
    ...
    return [...userPresets].sort((a, b) => {
      if (sortBy === "name") {
        ...
        const timeA = new Date(
          a.created_at || a.created_timestamp || 0,
        ).getTime();
        const timeB = new Date(
          b.created_at || b.created_timestamp || 0,
        ).getTime();
        return sortDir === "asc" ? timeA - timeB : timeB - timeA;
      } else {
        const timeA = new Date(
          a.download_timestamp || a.last_downloaded || 0,
        ).getTime();
        const timeB = new Date(
          b.download_timestamp || b.last_downloaded || 0,
        ).getTime();
        return sortDir === "asc" ? timeA - timeB : timeB - timeA;
      }
    });
  ...
  const togglePreset = (id: string) =>
    setExpandedPresets((p) => ({ ...p, [id]: !p[id] }));
    ...
    if (activeSession?.user) {
      ...
      if (userDiscordId) {
        ...
        authFetch(`/seedlist/count?creator_id=${userDiscordId}`)
          ...
          .catch((err) =>
            console.error("Error fetching user seeds count:", err),
          )
      ...
      >
            ...
            >
              ...
              {mostPlayedPreset ? (
                <>
                  ...
                  <div style={{ fontSize: "0.72rem", color: "#64748b" }}>
                    {mostPlayedCount} seed{mostPlayedCount !== 1 ? "s" : ""}{" "}
                    (from last 100 rolled)
                  </div>
                </>
              ) : (
                <div
        ...
        >
          ...
          >
            ...
            <span
              style={{
                fontSize: "1rem",
                fontWeight: "bold",
                color: userPresets.length >= 50 ? "#ef4444" : "#94a3b8",
              }}
            >
          ...
          {loadingPresets ? (
            ...
            >
              You have not saved any presets yet. Generate a seed and click
              &quot;Save as Preset&quot; to see them here!
            </div>
          ) : (
            <div
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          by {preset.creator_name || preset.creator || "You"} ·{" "}
                          {formatTruncatedDate(
                            preset.created_at || preset.created_timestamp,
                          )}
                        </div>
                      </div>
                      <div
        ...
        >
          ...
          >
            ...
            >
              {loadingSeeds ? "…" : userSeeds.length} of{" "}
              {loadingCount ? "…" : (totalSeedsCount ?? 0)} Total
            </span>
          </div>
          ...
          {userSeeds.length > 0 && !loadingSeeds && (
            ...
            >
              ...
              <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                ...
                >
                  Date Downloaded{" "}
                  {seedSortBy === "date" && (seedSortDir === "asc" ? "▲" : "▼")}
                </button>
                <button
                  onClick={() => {
                    if (seedSortBy === "tag") {
                ...
                >
                  Preset{" "}
                  {seedSortBy === "tag" && (seedSortDir === "asc" ? "▲" : "▼")}
                </button>
              </div>
            </div>
          )}
          ...
          {loadingSeeds ? (
            ...
            >
              You have not generated any seeds yet. Head over to the
              &quot;Generate&quot; tab and roll a seed!
            </div>
          ) : (
            <div
              style={{ display: "flex", flexDirection: "column", gap: "1rem" }}
            >
              {sortedSeeds.map((seed, idx) => {
                ...
                if (seed.timestamp) {
                  try {
                    formattedDate = new Date(seed.timestamp).toLocaleString(
                      undefined,
                      {
                        year: "numeric",
                        month: "short",
